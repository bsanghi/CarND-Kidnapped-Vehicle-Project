...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for (int i = 0; i < num_particles; ++i) {
    	...
    	for (int j = 0; j < observations.size(); j++) {
      		LandmarkObs tmp;
      		tmp.x = observations[j].x * cos_theta - observations[j].y * sin_theta + p_x;
      		tmp.y = observations[j].x * sin_theta + observations[j].y * cos_theta + p_y;
    	...
    	for (int j = 0; j < observations_map.size(); j++) {
    		float obs_m_x = observations_map[j].x;
      		float obs_m_y = observations_map[j].y;

      		Map::single_landmark_s landmark = map_landmarks.landmark_list.at(observations_map[j].id-1);
      		double x_term = pow(obs_m_x - landmark.x_f, 2) / (2 * pow(std_landmark[0], 2));
      		double y_term = pow(obs_m_y - landmark.y_f, 2) / (2 * pow(std_landmark[1], 2));
